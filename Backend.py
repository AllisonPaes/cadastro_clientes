...
class TransactionObject():
    ...
    def initDB():
        ...
        trans.execute('CREATE TEBLE IF NOT EXISTS clientes (id INTEGER PRIMARY KEY , none TEXT, sobrenome TEXT, email TEXT, cpf TEXT)')
        trans.persist()
        trans.disconnect()
    ...
    def search(nome="", sobrenome="", email="", cpf=""):
        ...
        trans.execute('SELECT * FROM clientes WHERE none=? or sobrenome=? or email=? or cpf=?', (nome, sobrenome, email, cpf))
        rows = trans.fetchall()
        trans.disconnect()
        return rows
